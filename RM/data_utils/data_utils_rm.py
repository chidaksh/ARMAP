...
def preprocess_for_reward_modeling(
    ...
    def _get_text(example: dict, output_key: str):
        ...
        if "image" in example:
            ...
            if image_to_caption_mapping is not None and "<obs>" in example["conversations"][0][0]["value"]:
                image_captions.append(image_to_caption_mapping[image])
                # random.shuffle(image_captions)

        assert _s[-1]["from"] == "gpt"
        if isinstance(example[output_key], list):
            _s[-1]["value"] = example[output_key][0]["value"]
            for idx in range(1,len(example[output_key])):
                _s.append(example[output_key][idx])
                if "image" in example[output_key][idx]:
                    image = example["image"]
                    if image_to_caption_mapping is not None and "<obs>" in example[output_key][idx]["value"]:
                        image_captions.append(image_to_caption_mapping[image])
        else:
            _s[-1]["value"] = example[output_key]["value"]
        # if len(image_captions)!=2:
        #     import pdb
        #     pdb.set_trace()
        return preprocess(
            [_s],
    ...
    # "size" (bsz, 2, seq_len)
    # import pdb; pdb.set_trace()
    # assert torch.equal(text_list_0["input_ids"], text_list_1["input_ids"]), "not equal"
    input_ids = [
        list(pair)
...
class BinaryRewardModelingDataset(Dataset):
    ...
    def __len__(self):
        ...
        #print("Debuging on 100 examples\n")
        #return 100 
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0] and "<image>" in sources[0]["conversations"][0]["value"]:
            image_file = self.list_data_dict[i]["image"]
            image_folder = self.data_args.image_folder
            # import pdb
            # pdb.set_trace()
            processor = self.data_args.image_processor
            img_full_path = os.path.join(image_folder, image_file)
            # img_full_path = "/nobackup/users/zfchen/cdl/VILA/av.png"
            # print("Debuging!\n")
            # print(img_full_path)
            ...
            if self.data_args.image_aspect_ratio == "pad":
                ...
                # crop_size = self.data_args.image_processor.image_size
        ...
        image_list1 = []
        image_list2 = []
        if "output_1" in self.list_data_dict[i]:
            for ins in self.list_data_dict[i]["output_1"]:
                if "image" in ins and "<image>" in ins["value"]:
                    image_file1 = self.list_data_dict[i]["image"]
                    image_folder1 = self.data_args.image_folder
                    image1 = Image.open(os.path.join(image_folder1, image_file1)).convert("RGB")
                    # crop_size = self.data_args.image_processor.image_size
                    # data_dict["image"] = torch.zeros(3, crop_size["height"], crop_size["width"])
                    image1 = image1.resize((384, 384))
                    image1 = processor.preprocess(image1, return_tensors="pt")["pixel_values"][0]
                    image_list1.append(image1)
        if "output_2" in self.list_data_dict[i]:
            for ins in self.list_data_dict[i]["output_2"]:
                if "image" in ins and "<image>" in ins["value"]:
                    image_file2 = self.list_data_dict[i]["image"]
                    image_folder2 = self.data_args.image_folder
                    image2 = Image.open(os.path.join(image_folder2, image_file2)).convert("RGB")
                    # crop_size = self.data_args.image_processor.image_size
                    # data_dict["image"] = torch.zeros(3, crop_size["height"], crop_size["width"])
                    image2 = image2.resize((384, 384))
        ...
        if "image" in self.list_data_dict[i] and "<image>" in self.list_data_dict[i]["conversations"][0]["value"]:
            # data_dict["image"] = image.to(torch.float16)
            data_dict["image"] = image.to(torch.bfloat16)
        # if len(image_list1)+len(image_list2) != 2:
        #     import pdb;pdb.set_trace()
        if len(image_list1) > 0:
            # data_dict["image1"] = [image.to(torch.float16) for image in image_list1]
            # data_dict["image2"] = [image.to(torch.float16) for image in image_list2]
            data_dict["image1"] = [image.to(torch.bfloat16) for image in image_list1]
            data_dict["image1"] = torch.stack(data_dict["image1"])

        if len(image_list2) > 0:
            data_dict["image2"] = [image.to(torch.bfloat16) for image in image_list2]
            data_dict["image2"] = torch.stack(data_dict["image2"])
...
class DataCollatorForBinaryRewardModelingDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        id, index_0, index_1, choice = tuple(
            torch.stack([instance[key] for instance in instances])
            for key in ("id", "index_0", "index_1", "choice")
        )
        ...
        if "image" in instances[0]:
            ...
            batch["images"] = []
        if "image1" in instances[0]:
            try:
                image1 = [instance["image1"] for instance in instances]
            except:
                import pdb
                pdb.set_trace()
            if all(x is not None and x.shape == image1[0].shape for x in image1):
                batch["image1"] = torch.stack(image1)
            else:
                batch["image1"] = image1
        else:
            batch["image1"] = []
        if "image2" in instances[0]:
            image2 = [instance["image2"] for instance in instances]
            if all(x is not None and x.shape == image2[0].shape for x in image2):
                batch["image2"] = torch.stack(image2)
            else:
                batch["image2"] = image2
        else:
            batch["image2"] = []
