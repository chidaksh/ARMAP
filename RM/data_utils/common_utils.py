...
def preprocess_llama_2(
    ...
    for c, conversation, target in zip(
        ...
        assert final_query_len <= query_len, "query len is too long!"
        # if final_response_len > response_len:
        #     print(f"{final_response_len} > {response_len}")
        assert final_response_len <= response_len, "response len is too long!"
...
def preprocess_v1(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if "<obs>" in value:
                # print(f"{obs_idx} {len(image_captions)}")
                value = value.replace("<obs>",image_captions[obs_idx][0])
                # print(image_captions[obs_idx][0])
                obs_idx+=1
        ...
        if (
            ...
            reward_model_prompt_per_example = reward_model_prompt_per_example.format(
                factual_prompt=factual_prompt,factual_prompt1=factual_prompt
            )
        
        elif(
            image_captions is not None
            and r"<image>" in reward_model_prompt_per_example
        ):
            factual_prompt = FACTUAL_PROMPT
            for caption in image_captions:
                factual_prompt = factual_prompt + f"  - {caption}\n"
            reward_model_prompt_per_example = reward_model_prompt_per_example.format(
                Intent = source[0]["value"],observation0="image_captions[0]",observation1="image_captions[1]",response0=source[1]["value"],response1=source[3]["value"]
            )

            # print(reward_model_prompt_per_example)
    ...
    for c, conversation, target in zip(
        ...
        # if final_query_len>query_len:
        #     print(f'final_query_len :{final_query_len}')
        assert final_query_len <= query_len, "query len is too long!"
        # if final_response_len > response_len:
        #     print(f"{final_response_len} > {response_len}")
        assert final_response_len <= response_len, f"response len is too long! {final_response_len}"
