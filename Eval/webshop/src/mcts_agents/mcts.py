...
from src.mcts_agents.utils import multigpu_breakpoint
import pdb
    ...
    for _ in tqdm(range(ag.rollouts), desc="Rolling out", leave=False):
        ...
        while select:
            if type(node) == DecisionNode: # DecisionNode
                ...
                state_p, reward, terminal, reward_gt = env.transition(node.parent.state, node.action, ag.is_model_dynamic)
        ...
        if ag.default_policy is None:
            ...
            while (not terminal) and (t < ag.horizon):
                ...
                state, reward_pd, terminal, reward_gt = env.transition(state, action, ag.is_model_dynamic)
            ...
            if not node.is_terminal:
                ...
                state, reward_pd, terminal, reward_gt = ag.default_policy.rollout_sequence(state)
                
                estimate = reward_pd

                # to modify the state to the sequence of the history state and actions
                # log the state history
                ag.rolled_out_trajectories.append(state)
                ag.rolled_out_rewards.append(estimate)
                ag.rolled_out_rewards_gt.append(reward_gt)

                # also log the terminal state to current nodes
                # log the state history
                node.info['terminal_state'] = state
            else:
